...
namespace OpenWeatherMap.Web.Tests
{
    ...
    public class OpenWeatherMapClientTest
    {
        ...
        [TestMethod]
        public void TestGetWeatherData_GivenLocation_ValidApiKey_MinimalSettings_ReturnsWeatherData()
        {
            var owmClient = new OpenWeatherMapClient(_apiKey);

            var data = owmClient.GetWeatherData("Osaka").Result;
            ...
            Assert.IsTrue(data.Hourly.Count > 0);
            Assert.IsTrue(data.Daily.Count > 0);
            Assert.IsTrue(data.Latitude != 0);
            Assert.IsTrue(data.Longitude != 0);
        }

        [TestMethod]
        public void TestGetWeatherData_GivenLocation_ValidApiKey_AddedSettings_ReturnsWeatherData()
        {
            var owmClient = new OpenWeatherMapClient(_apiKey);
            var excludedData = new ExcludeData[] { ExcludeData.Minutely, ExcludeData.Hourly, ExcludeData.Alerts };

            var data = owmClient.GetWeatherData("Osaka", excludedData, UnitsOfMeasurement.Metric).Result;
        ...
        [TestMethod]
        public void TestGetWeatherData_GivenLocation_InvlidApiKey_ReturnsError()
        {
            var owmClient = new OpenWeatherMapClient("");
        ...
        [TestMethod]
        public void TestGetWeatherDataForDate_GivenLocation_ValidApiKey_MinimalSettings_ReturnsWeatherData()
        {
            var owmClient = new OpenWeatherMapClient(_apiKey);

            var data = owmClient.GetWeatherDataForDate("Osaka", DateTime.Now).Result;
            ...
            Assert.IsTrue(data.Current[0].UVIndex >= 0.0);
            Assert.IsTrue(data.Current[0].CloudCoverage >= 0);
            Assert.IsTrue(data.Latitude != 0.0);
            Assert.IsTrue(data.Longitude != 0.0);
        }

        [TestMethod]
        public void TestGetWeatherDataForDate_GivenLocation_ValidApiKey_AddedSettings_ReturnsWeatherData()
        {
            var owmClient = new OpenWeatherMapClient(_apiKey);

            var data = owmClient.GetWeatherDataForDate("Osaka", DateTime.Now, UnitsOfMeasurement.Metric, DataLanguage.Japanese).Result;
        ...
        [TestMethod]
        public void TestGetWeatherDataForDate_GivenLocation_InvlidApiKey_ReturnsError()
        {
            var owmClient = new OpenWeatherMapClient("");
        ...
        [TestMethod]
        public void TestGetDailyAggregateData_GivenLocation_ValidApiKey_AddedSettings_ReturnsDailyAggregateData()
        {
            var owmClient = new OpenWeatherMapClient(_apiKey);

            var data = owmClient.GetDailyAggregateData("Osaka", DateTime.Now, "+02:00", UnitsOfMeasurement.Metric, DataLanguage.Japanese).Result;
        ...
        [TestMethod]
        public void TestGetDailyAggregateData_GivenLocation_InvlidApiKey_ReturnsError()
        {
            var owmClient = new OpenWeatherMapClient("");

            try
            {
                var data = owmClient.GetDailyAggregateData("Osaka", DateTime.Now).Result;
                Assert.Fail("Test case should not go here");
            }
            catch (AggregateException agEx)
            {
